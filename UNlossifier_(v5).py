...
def coherence_loss(pred, target): # not really stft, just for frizz.
    pred_lr = pred[:, :2]
    target_lr = target[:, :2]
...
def train(args):
    ...
    loader = torch.utils.data.DataLoader(
        ...
        shuffle=False,
        num_workers=2,            # non mettere zero, hai prefetch, ti darà error. prova 2, non 8
        persistent_workers=True,  # evita il ri-spawn a ogni epoch
        prefetch_factor=2,
