...
- **model_mp3_64k_44100_epoch997.safetensors** (Epoch 997 l_lr: 0.005969 l_ms: 0.004224 l_stft: 0.021610 l_consistency: 0.000126 TOTAL: 0.014579)-
  ...
  This model is optimal for mp3 encoded files with libmp3lame (FFmpeg LAME encoder) at CBR but still far away from clean restoration (clicks, pops, noise artifacts and intersample peaks, read tip below for more info.).
...
**Info:** A model trained on 64 kbps compressed audio can be applied to higher-quality sources (e.g. 320 kbps “near-lossless” MP3). In this case, the model does not simply restore missing information; it may reinterpret existing spectral content, subtly altering timbre and texture. In some cases, what is actually valid signal content may be treated as compression artifacts and reshaped accordingly.

**Info:** **At 64 kbps MP3**, the notion of “silence” is no longer truly silent in a statistical sense. The codec introduces low-level structured artifacts even in regions that are perceptually quiet, including residual pre-echo, aggressive noise shaping, and unstable psychoacoustic masking behavior. As a result, what appears to be low-energy audio still contains organized spectral residue that is not random noise but compression-induced structure.
...
### Update v5 (14/06/2026): Refined approach and optimized training
* **Replaced the multi-resolution STFT loss with a waveform-domain coherency loss**: v4 relied on a computationally expensive multi-resolution STFT loss to compare spectral content across several FFT sizes. While effective, it significantly increased training cost and memory usage. v5 replaces it with a lightweight waveform-domain coherency_loss that evaluates local energy stability, temporal correlation and short-range waveform behavior directly in the time domain.
* **Removed frequency-weighted psychoacoustic spectral matching**: The STFT loss in v4 included custom low-frequency weighting, log-magnitude matching and spectral gradient analysis designed to emphasize perceptually important regions, v5 abandons handcrafted spectral weighting in favor of learning temporal structure directly from the waveform.
* **Added multi-scale temporal coherency analysis**: The new coherency loss evaluates signal behavior across multiple temporal scales, measuring whether local waveform dynamics evolve similarly to the clean reference rather than focusing on exact sample values alone so the model is encouraged to preserve short and medium range temporal relationships.
* **Context-aware training and inference remain fully compatible**: v5 preserves the overlap-add inference pipeline, Mid/Side representation, consistency constraints, cached audio preprocessing and context-window strategy introduced in previous versions, this means existing workflows remain compatible while benefiting from the new training objective.
* **PROS**: Faster training. Lower VRAM usage. Simpler optimization pipeline. No FFTs computation during training. Fewer assumptions about perceptual importance. Reduced implementation complexity. Better temporal stability. Encourages natural waveform evolution. Less sensitive to minor phase differences.
* **CONS**: Less explicit frequency-domain supervision. May require additional tuning for highly tonal material. Fine spectral details are left entirely to the network and training data. Improvements may be more perceptual than measurable. Training speed improvements do not automatically guarantee better restoration quality. Architectural limitations of the core network remain unchanged.
